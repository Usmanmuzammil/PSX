...
function getSingleUserDetails($id)
    {
        $depositDetails = TransactionTable::where('type', 'Deposit')->where("user_id", $id)->where('deposit_id', '>', 0)->sum('avaiable_amount');
        $withdrawDetails = TransactionTable::where('type', 'withdraw')->where("user_id", $id)->where('withdraw_id', '>', 0)->sum('avaiable_amount');
        $investmentDetails = TransactionTable::where('type', 'invest')->where("user_id", $id)->where('investment_id', '>', 0)->sum('avaiable_amount');


        return response()->json([
            'total_deposit' => $depositDetails,
            'total_withdraw' => $withdrawDetails,
            'total_investment' => $investmentDetails,
